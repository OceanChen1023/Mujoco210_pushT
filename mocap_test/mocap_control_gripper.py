...
initial_pos = [0, -1.35, 1.35, -1.6, -1.5, 1.5, #0.8, -1.35, 1.35, -1.6, -1.6, 1.5,#robot arm
               0.02, 0, 0, 0.6,
               0.02, 0, 0, 0.6]
...
def main():
    ...
    # load model
    model = load_model_from_path("./UR5_gripper.xml")
    # model = load_model_from_path("./lab_env.xml")
    sim = MjSim(model)
    ...
    #sim.data.set_mocap_pos("mocap", np.array([0.0, 0.0, 1.436]))  #+ np.array([0.3, 0 , -0.4]))
    #sim.data.set_mocap_quat("mocap",np.array([0.71,0,0.71,0]))
    #sim.data.qpos[0:6]=np.array([0.113,-1.363,1.59,-1.046,-0.26,-0])


    print("sim.data.qpos length=",sim.data.qpos.shape)
    sim.data.qpos[:]=np.zeros(sim.model.nq)
    #sim.data.qpos[0:6] = np.array([-0.08, -1.35, 1.35, -1.6, -1.6, 1.5])
        
    sim.step()

    T_block_site_id = sim.model.site_name2id("T_block_site")
    # T_block_site_id = sim.model.site_name2id("target_site_1")
