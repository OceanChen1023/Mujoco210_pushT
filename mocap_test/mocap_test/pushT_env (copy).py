...
class PushEnv(gym.Env):
    ...
    def reset(self):
        ...
        # self.block_init_pos[0]=np.random.uniform(0.5,0.8)
        # self.block_init_pos[1]=np.random.uniform(-0.35,0.35)
        # self.block_init_pos[2]=0.912
    ...
    def _set_robot_pose(self):
        ...
        self.sim.data.set_mocap_pos("mocap", np.array([0.2, 0.0, 1.436]))  #+ np.array([0.3, 0 , -0.4]))
    ...
    def _calculate_reward(self,obs,pre_pos,cur_pos):
        ...
        goal_pos = self.block_init_pos.copy()+np.array([0, 0, 0.4]) #np.array([0.2, 0.0, 1.5]) #self.block_init_pos
